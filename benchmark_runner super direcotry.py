...
# --- CONFIGURATION ---
BASE_DIRS = [r"C:\am21\TEIAS\designedBenchmarks"]
...
CSV_FILE_PATH = 'rounding_benchmark_results.csv'
SOLVER = "spacer"
...
def main():
    ...
    for b_dir in BASE_DIRS:
        if not os.path.exists(b_dir):
            ...
            continue
        for folder1 in os.listdir(b_dir):
            b_dir_sub_folder =os.path.join(b_dir, folder1)
            if not os.path.isdir(b_dir_sub_folder):
                print(f"Warning: Directory not found: {b_dir}")
                continue
            for folder in os.listdir(b_dir_sub_folder):
                if GET_CEX and os.path.isdir(os.path.join(b_dir, folder)):
                    cex_dir = os.path.join(b_dir_sub_folder, folder, CEX_DIR_NAME)
                ...
                for rounding_enc in ENCODINGS:
                    for norm_enc in ENCODINGS:
                        tasks.append((b_dir_sub_folder, folder, rounding_enc, norm_enc))
