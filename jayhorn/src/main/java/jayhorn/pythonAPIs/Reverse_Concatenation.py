...
def reverse_concat(X_v, X_m, n_A, n_B):
    """
    Recover operands A (upper n_A bits) and B (lower n_B bits)
    from concatenated ternary vector X of width n_A + n_B.
    Returns (A_v, A_m, B_v, B_m) as ints, or raises on contradiction.
    """
    N = n_A + n_B
    ...
    val_X = BitVecVal(X_v, N)
    msk_X = BitVecVal(X_m, N)
    ...
    raise Exception("Contradiction: cannot reverse concatenation.")
...
if __name__ == "__main__":
    ...
    A_v, A_m, B_v, B_m = reverse_concat(X_v, X_m, n_A, n_B)
    print(f"{A_v},{A_m},{B_v},{B_m}")
