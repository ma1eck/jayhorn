...
def infer_mul_result_mask(Avalue_str, Amask_str, Bvalue_str, Bmask_str):
    """
    Infer the most precise masked result of multiplication of two masked bit-vectors.
...
def test():
    ...
    for Avalue_str, Amask_str, Bvalue_str, Bmask_str in example_input:
        result_value, result_mask = infer_mul_result_mask(Avalue_str, Amask_str, Bvalue_str, Bmask_str)
...
if __name__ == "__main__":
    ...
    val, mask = infer_mul_result_mask(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
    print(f"{val},{mask}")
